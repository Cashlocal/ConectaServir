...
export default function VoluntariosAdminPage() {
  ...
  async function salvar() {
    ...
    try {
      ...
      if (isEditar) {
        setVoluntarios((prev) => prev.map((v) => (v.id === data.id ? data : v)));
      } else {
        setVoluntarios((prev) =>
      ...
      <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-base text-[#475569]">
            Gerencie os voluntários cadastrados
          </p>
        </div>
        <button
          type="button"
                      ...
                      <label className="inline-flex cursor-pointer items-center" title={v.status === "Ativo" ? "Clique para inativar" : "Clique para ativar"}>
                        ...
                        <span
                          className={`relative inline-block h-5 w-9 rounded-full transition-colors duration-200
                            ...
                            ${v.status === "Ativo" ? "bg-[#1d4ed8]" : "bg-[#cbd5e1]"}`}
                        >
                          <span
                            className={`absolute top-0.5 h-4 w-4 rounded-full bg-white shadow transition-transform duration-200
                              ${v.status === "Ativo" ? "translate-x-[18px]" : "translate-x-0.5"}`}
                          />
                        </span>
                      </label>
                    </td>
